/**
 * 将所有SVG格式的历史国旗转换为PNG格式
 * 使用方法: 
 *   node scripts/convert_flag_history_to_png.js          # 转换并保留SVG
 *   node scripts/convert_flag_history_to_png.js --clean   # 转换后删除SVG文件
 *   node scripts/convert_flag_history_to_png.js --force   # 强制重新转换已存在的PNG
 * 
 * 需要安装依赖:
 *   npm install sharp
 */
...
async function convertSvgToPng(svgPath, pngPath) {
  ...
  try {
    await sharp(svgPath, {
      ...
      .resize(800, 533, { // 国旗通常是3:2比例，所以使用800x533（近似3:2）
        fit: 'contain',
...
async function convertAll() {
  ...
  const svgFiles = files.filter(file =>
    file.startsWith('flag_history_') && file.endsWith('.svg')
  );

  if (svgFiles.length === 0) {
    console.log('没有找到SVG格式的历史国旗文件');
    return;
  }

  console.log(`找到 ${svgFiles.length} 个历史国旗SVG文件，开始转换...\n`);
  ...
  for (const svgFile of svgFiles) {
    ...
    process.stdout.write(`正在转换 ${svgFile}... `);
